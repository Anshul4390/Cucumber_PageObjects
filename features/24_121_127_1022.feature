Feature: Regression Tests
  ...
  # @testCaseId:6248
  Scenario Outline: Verify user can access the Defect form - @testCaseId:6248
    Given I am on service now home page 
    When I type the term <Filter> in filter text box
    Then I see <Module> module and other links inside the left pane window
    When I click on <Module_Link> link in the left navigation menu
    Then I see a new Defect form opens in the right pane enabling the user to enter a defect

    Examples:
      |Filter|Module|Module_Link|
      |Defect|Defect|Create New|
